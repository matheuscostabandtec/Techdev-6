...
function iniciar_escuta() {
    ...
    porta_serial.list().then(entradas_seriais => {
        ...
        var entradas_seriais_arduino = entradas_seriais.filter(entrada_serial => {
            console.log(`placa: ${entrada_serial.vendorId} / ${entrada_serial.productId}`);
...
function registrar_leitura(temperatura) {
    ...
    banco.conectar().then(() => {
        ...
        return banco.sql.query(`INSERT into dados (temperatura,momento)
                                values (${temperatura}, CONVERT(Datetime, '${agora()}', 120));`);

// INSERT into dados (temperatura,momento) values (${temperatura}, CONVERT(Datetime, '${agora()}', 120));
// delete from dados where idDado > 0
...
if (gerar_dados_aleatorios) {
	...
	setInterval(function() {
		...
		registrar_leitura(((Math.min(Math.random()*7) + 18) * 0.30).toFixed(2))
	}, 5000);
} else {
